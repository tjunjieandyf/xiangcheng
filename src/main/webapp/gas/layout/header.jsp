...
%><%
    ...
    /**图片收藏是否显示*/
    String imgCollectVisible = GgdmService.getInstance().getDmnrByDmjAndDm("CPPT", "IMGCOLLSHOW");
    request.setAttribute("imgCollectVisible", imgCollectVisible);
%>
...
<head>  
    ...
    <%--平台UI样式 --%>
    <nbean:css foldar="skins" path="${skin}/style/gas/index/style.css"/>
    <nbean:css foldar="skins" path="${skin}/style/gas/index/icon.css"/>
    <nbean:css foldar="skins" path="${skin}/style/gas/index/reset.css"/>
    <%-- easyUI样式文件 --%>
    ...
    <%--空气质量预报预警系统 公用js --%>
    <nbean:css foldar="scripts" path="gas/common/common.js"/>
    <%--空气质量预报预警系统 公用样式 --%>
    <nbean:css foldar="skins" path="${skin}/style/gas/system.css"/>
    ...
    <%-- layer插件 --%>
    <nbean:css foldar="platforms" path="plugins/layer/layer.js"/>
    <nbean:css foldar="platforms" path="plugins/layer/extend/layer.ext.js"/>
    <nbean:css foldar="platforms" path="plugins/layer/layerTool.js"/>
    ...
    <nbean:css foldar="scripts" path="plugin/player/index.js"/>
    <!-- zTree -->
      <c:if test="${requestScope.isZTree eq true}">
        <nbean:css foldar="scripts" path="plugin/ztree/js/jquery.ztree.all-3.5.js" />
        <nbean:css foldar="scripts" path="plugin/ztree/css/zTreeStyle/zTreeStyle.css" />
        <nbean:css foldar="scripts" path="/plugin/ztree/js/ztreeutil.js" />
      </c:if>
    <!-- echarts图片后台保存:base64 -->
    <%-- <nbean:ajaxUrl var="saveAsImgBase64" clazz="com.szboanda.gas.imgCollect.service.EchartsCollectService" method="saveAsImg" /> --%>
    <script type="text/javascript">
    	//echarts图片收藏后台url
    	var saveEchartsImgUrl_base64 = '${saveAsImgBase64}';
    	var imgCollectVisible = '${imgCollectVisible}';
        //禁止自动渲染
        $.parser.auto = false;
        ...
        $(function(){
             <c:if test="${ not empty param.load}" >
