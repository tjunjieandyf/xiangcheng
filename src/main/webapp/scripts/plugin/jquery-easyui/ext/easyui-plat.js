...
plat.datagrid={
	...
	setFieldValue:function(obj, table, params){
		if(power.isEmpty(params)){
			...
			Layer.showAlert('请确认tableInfo信息是否填写！');
			return;
		}
		table = power.getObject(table);
		...
		if(/^[0-9]*$/.test(obj)){
			...
			if(obj.length == 0){
				...
				Layer.showAlert("参数一设置有误，请设置datagrid的下标或子表中控件对象。");
				return;
			};
		}
	...
	setFieldData:function(obj, table, params){
		if(power.isEmpty(params)){
			//power.alert("提示信息", "请先设置Filed或Value值", "info");
			Layer.showAlert("请先设置Filed或Value值");
			return;
		}
		obj = power.getObject(obj);
		table = power.getObject(table);
	...
	getFieldValue:function(obj, table, params){
		if(power.isEmpty(params)){
			//power.alert("提示信息", "请先设置需要得到的字段值", "info");
			Layer.showAlert("请先设置需要得到的字段值");
			return;
		}
		table = power.getObject(table);
		var index = "";
	...
	save:function(table, fn, actionUrl){
		...
		actionUrl = power.isEmpty(actionUrl) ? plat.datagrid.getActionUrl() : actionUrl;
		this.endEdit($table);
		...
		if(chgLen){
			...
			if(insert){
				...
				formResult = power.ajaxRequest(actionUrl, {formItems:inserts, methodName:"ajaxInsert",ajax_dataType:'text'});
			...
			if(update) {
				...
				formResult = power.ajaxRequest(actionUrl, {formItems:updates, methodName:"ajaxUpdate",ajax_dataType:'text'});
				if(formResult.startWith("e")) formResult = formResult.substring(1);
				var formResultObj = new FormResult(formResult);
				if("exception" == formResultObj.operateResult){
					rollback();
	...
	del:function(table, fn, actionUrl){
		...
		if(idField){
			...
			if(selRowLength > 0){
				...
				Layer.showConfirm('是否确定删除选定记录?',function(){
					...
					if(del){
						...
						var formResult = power.ajaxRequest(actionUrl, {formItems:deletes, methodName:"ajaxDelete",ajax_dataType:'text'});
						if(formResult.startWith("e")) formResult = formResult.substring(1);
						var formResultObj = new FormResult(formResult);
						if("exception" == formResultObj.operateResult) {
							//提示异常信息
						...
						//power.msgTip($table.datagrid('getPanel'), {msg:formResultObj.message});
						Layer.showSucAlert(formResultObj.message);
						//$table.datagrid("acceptChanges");
						if($.isFunction(fn)) fn();
					};
				});
			}else{
				Layer.showAlert("请先选择删除行！");
				//power.msgTip($table.datagrid('getPanel'),{msg:"请先选择删除行！", level:"info"});
			}
		}else{
			Layer.showFailAlert('datagrid未配置idField属性值');
	 ...
	 parseTableInfo:function(table){
		...
		var tableInfo = $options.tableInfo;
      	if(tableInfo){
      		...
      		Layer.showFailAlert('请确认tableInfo信息是否填写！');
      		return false;
      	}
      	return formItems;
	 },
...
plat.tabs={
	...
	enableTab: function(obj,parent){
		if(parent){
			parent.plat.tabs.enableTab(obj);
		}else{
			obj = obj || "#tabs";
			var $tabs = power.getObject(obj);
			try{
	...
	disableTab: function(obj,parent){
		if(parent){
			parent.plat.tabs.disableTab(obj);
		}else{
			obj = obj || "#tabs";
			var $tabs = power.getObject(obj);
			try{
