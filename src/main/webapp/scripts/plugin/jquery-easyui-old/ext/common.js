...
var power = {
	 ...
	 * 									replaceAll true of false, 是否替换所有的值，
	 ...
	 * 									
	 * 如：{form:'#form', method:'update', replaceAll:true, columns:'', fn: function(){alert(1)}}
	 */
	asynSubmit:function(obj){
		...
		if(power.isNotEmpty(obj)){
			...
			replaceAll = obj.replaceAll || replaceAll;
				...
				success:function(formResult){
					...
					try{
						...
						if("success" == formResultObj.operateResult){
							...
							if($.isFunction(fn) && isRunFn) fn(formResult[0]);
	//						power.monitorDataChange(); 
						}
						if("success" == formResultObj.operateResult){
							power.msgTip($form, {msg:power.isNotEmpty(successMsg) ? successMsg : formResultObj.message});
						}else if("fail" == formResultObj.operateResult){
							power.msgTip($form, {msg:power.isNotEmpty(failMsg) ? failMsg : formResultObj.message, level:'fail'});
	 ...
	 *  								isLoadSuccess 如果更新datagrid此参数有效，true执行定义的onLoadSuccess事件，false则不执行
	 *  如：{parent:window.parent, grid:"#grid", form:"#form", choose:"treegrid", fn:function(){},beforeReload:function(){}}
	 */
	...
	updateDatagrid:function(param){
		var grid = "#datagrid", fn, isLoadSuccess;
	...
	queryParams:function(qForm){
		...
		if(paramArray != ''){
			for (var i = 0; i < paramArray.length; i++) { 
				queryObj[paramArray[i].name] = paramArray[i].value;
			}
		}
		$(".searchInput", qForm).each(function(){
			if(this.value == ""){
	...
	monitorDataChange:function(forms) {
		...
		if(this.forms.indexOf(",") != -1){
			$(this.forms.split(",")).each(function(index, value){
				getInitValue(value);
			});
		}else{
	...
	isChange:function() {
		var change = false;
		if(power.forms.indexOf(",") != -1){
			$(power.forms.split(",")).each(function(index, value){
	...
	tooltip: function(obj){
		...
		obj = obj.length > 0 ? obj : $("input[type='text']");
		obj.bind("keyup",function(e){
			var el = this;
			var content = '<div style="font-size: 15px;color: red;height:25px;font-weight: 200">' + $(el).val() + '</div>';
			$(el).tooltip({
				position: 'top',
				deltaX: ($(el).width()/2) * -1,
				content: content,
	 ...
	 * 单独启动某个控件的验证
	 * 
	 * @param obj 需要启动验证的对象 如：#test
	 * @param isFocus 验证失败后是否需要控件得焦，默认为true
	 */
	enableValid:function(obj, isFocus){
		obj = power.getObject(obj);
		isFocus = power.isEmpty(isFocus) ? true : isFocus;
		obj.validatebox("enableValidation");
		if(isFocus){
			if(!obj.validatebox("isValid")) obj.focus();
		}else{
					...
					if (label != null && "*" === label.charAt(0)) {
						if(!isExistName(name)){
							$("input:radio[name=" + name + "]").each(function(){
								$(this).addClass(validCls);
								$(this).attr("validType", "checked");
							});
						}
					}
				}else if("checkbox" == type){
 ...
 * 调用人员树的方法
 * 
 * @param 	maxNum 当前返回页面的最大个数，Integer类型，默认为12，null为默认值
 ...
 * @return 	['人员名称','人员ID','所在部门ID']
 * 
 * @since 	V0.1.0
 */
power.userTree = function(maxNum,atoz,keyStr,valueStr,departmentStr){debugger;
	var _maxNum = !$.isNumeric(maxNum) ? 12 : (maxNum > 0 ? maxNum : 0);
	var _sort = Boolean(atoz) ? 'atoz' : 'classify';
	var url = ctx + "/lims/ggzj/bms/rys.jsp?sort=" + _sort + "&maxNum=" + _maxNum;
	var sFeatures = "dialogHeight:556px;dialogWidth:915px;status:no;help:no;scroll:no;resizable:no;location:no;toolbar:no;";
	var arr = window.open(url,{fxxmdm:$.trim(keyStr),fxxmmc:$.trim(valueStr),lx:$.trim(departmentStr)}, sFeatures);
	if(arr == undefined){ //谷歌
 ...
 * @since 	V0.1.0
 */
power.departmentTree = function(hasCheckbox,onlyLeafCheck,cascadeCheck,defaultArr){
 ...
 *  showBz: //0不显示备注列，1显示，默认不显示
 *  showSel： //0 不显示选择按钮，1显示，默认不显示
 *  canInsert： //0，不出现新增按钮，1显示，默认不显示
 *  canUpdate; //0，不能编辑，1，可以编辑，默认0
 *  canDelete;  //0，不能删除；1，可以删除，默认0
 * 
 ...
 * @since 	V0.1.0
 */
power.showGgdmz = function(args,sel){
	  var sFeatures = "dialogHeight: 400px;dialogWidth:750px;scroll:on;";
...
/*
//屏蔽全选，除INPUT框外
window.document.document.onselectstart = function(e){
