...
var easyuiErrorFunction = function(XMLHttpRequest) {
	...
	if(power.isNotEmpty(formResult)){
	 			...
	 			success:function(data){
					...
					//无数据时显示无数据
	 				if(!(($.isArray(data) && data.length > 0) || (data.rows && data.rows.length > 0))){
	 					...
	 					var html = "<div style=\"font-size: 13px;color:red;\">暂无数据</div>";
...
$.extend($.fn.datagrid.defaults.editors, { 
	...
	select:{
		init: function(container, options){
			...
			var option = "";
			$(data).each(function(){
				option += "<option value='" + this.DM + "'>" + this.DMNR + "</option>";
			});
			var $select = $("<select > ");
...
/*$.extend($.fn.tabs.defaults,{
	onUnselect:function(title, index){
		...
		if(power.isNotEmpty(frame[0])){
			var win = frame[0].contentWindow;
			if($.isFunction(win.isChange)){
				if(win.isChange()){
					power.confirm("提示信息","当前页面数据已有修改，是否保存。",function(r){
						if(r){
