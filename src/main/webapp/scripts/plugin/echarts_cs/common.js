var echartsUtil = {
	...
	echartsAjaxLoad : function(requestEntity) {
		...
		gas.ajaxRequest(requestEntity.ajax_url, requestEntity.ajax_Entity);
	},
	/**
	 * ajax回调函数	
	 * 
...
echartsUtil.radar_default = function(result) {
	...
	if (result.polar) {
		/*radar=result.polar;*/
		var textList=[];
		//给数据加上min:0 属性，防止出现负数轴
		$(result.polar[0].indicator).each(function(){
			var obj={
					name:this.text,
		...
		radar[0].indicator = textList;
	} else if (result.radar) {
	/*	radar=result.radar;*/
		var textList=[];
		//给数据加上min:0 属性，防止出现负数轴
		$(result.radar[0].indicator).each(function(){
			var obj={
					name:this.text,
					axisLabel:this.axisLabel,
					min:0
