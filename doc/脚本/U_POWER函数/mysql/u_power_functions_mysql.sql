...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION U_POWER.`p_to_char`(fdate date, format varchar(255)) RETURNS varchar(255) CHARSET utf8
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION U_POWER.`p_to_date`(fdate VARCHAR(255), format VARCHAR(255)) RETURNS date
BEGIN
	set format = REPLACE(format, 'yyyy', '%Y');
	set format = REPLACE(format, 'YYYY', '%Y');
	set format = REPLACE(format, 'YY', '%y');
